# Yin Yang Origin and Symbolism

[Live site](https://raphaelasanti.github.io/yin-yang/) • [Full ebook (The End of Time)](https://raphaelasanti.github.io/the-end-of-time/) • [License](./LICENSE)

A concise structural overview of the yin–yang symbol and two interpretations that are not opposites. This project presents both the Western dualistic reading and the compatibility‑based reading, along with historical context and structural analysis.

---
[![License: CC BY-ND 4.0](https://img.shields.io/badge/License-CC%20BY--ND%204.0-lightgrey.svg)](http://creativecommons.org/licenses/by-nd/4.0/)
[![GitHub Pages](https://img.shields.io/badge/site-GitHub%20Pages-green.svg)](https://raphaelasanti.github.io/yin-yang/)

---

## Table of contents
- [About](#about)
- [Files / Links](#files--links)
- [How to view](#how-to-view)
- [License](#license)
- [Author](#author)
- [Donate](#donate)
- [Contribute](#contribute)
- [Related pages](#related-pages)

## About
This repo contains a focused HTML page presenting two structural interpretations of the yin–yang symbol: the Western good/evil dualistic reading and the compatibility‑based reading. It includes historical notes, scriptural parallels, and examples of non‑dualistic mutual beneficiality.

## Files / Links
- index.html — Primary page (HTML)  
  https://raphaelasanti.github.io/yin-yang/
- Image asset  
  yinyang.jpg
- Source repo  
  https://github.com/raphaelasanti/yin-yang

## How to view
Open the live site URL above (GitHub Pages). The page is served directly via GitHub Pages — no installation required.

## License
This work is distributed under the license in the repository: [LICENSE](./LICENSE)

## Author
Raphael E. Asanti — August 2026

## Donate
This work is sustained entirely by voluntary contributions. Thank you for considering support.

- Cash App: https://cash.app/$phirunltd  
- Gumroad (optional donation when downloading): https://phirun4.gumroad.com/l/zsmzjh

(If you prefer not to donate, the content is available for free — see "Related pages".)

## Contribute
- Share the public page on social media to extend its reach: https://raphaelasanti.github.io/yin-yang/  
- Report issues or suggest edits via GitHub issues in this repo. Pull requests welcome for documentation fixes or accessibility improvements.

## Related pages
- Main Index: https://raphaelasanti.github.io/
- Full eBook: https://github.com/raphaelasanti/the-end-of-time/
- Thamud and the Camel of God: https://github.com/raphaelasanti/thamud-and-the-camel-of-god/
- The Eye of the Needle: https://raphaelasanti.github.io/the-eye-of-the-needle/
- Laughter Is Better Than Sex: https://raphaelasanti.github.io/laughter-is-better-than-sex/

